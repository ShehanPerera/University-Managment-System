...
error_reporting(1);
 ...
 {
 $old=md5($op);
	$que=mysqli_query($con,"select * from instructor where email='".$_SESSION['staff']."' and pass='$old'");
	...
	$row= mysqli_num_rows($que);
	 ...
	 { $new=md5($np);
		if($np==$cp)
		{
		mysqli_query($con,"update instructor set pass='$new' where email='".$_SESSION['staff']."'");
