...
error_reporting(1);
 ...
 if(isset($updpass))
 {
	...
	$que=mysqli_query($con,"select * from student where email='".$_SESSION['student']."' and pass=md5('$op')");
	...
	$row= mysqli_num_rows($que);
	 ...
	 if($row)
	 {
		if($np==$cp)
		{
		mysqli_query($con,"update student set pass=password('$np') where email='".$_SESSION['student']."'");
