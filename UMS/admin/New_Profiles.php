				...
				<li class="pull-right">
					...
					<div class="row">
						<div class="header " data-original-title="">
							<h2><i class="glyphicon glyphicon-user"></i>New User Accounts</h2>
						</div>
					</div>    
					<?php //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++   Tables
						include('../connection.php');
						if(($_SESSION['res']=="mainadmin")||(in_array("mainadmin",$_SESSION['position1'])))
						{
							$query ="SELECT * FROM user WHERE accept !='true' or action !='no' ORDER BY user_id ASC";
						...
						if(($_SESSION['res']=="hadmin")||(in_array("hadmin",$_SESSION['position1'])))
						{
							$query ="SELECT * FROM user WHERE accept !='true' AND (position = 'warden' or position = 'subwarden' or position ='hadmin' ) ORDER BY user_id ASC";
							$result1=mysql_query($query);
							$i =0;
							$count=0;
						}
						if(($_SESSION['res']=="madmin")||(in_array("madmin",$_SESSION['position1'])))
						{
							$query ="SELECT * FROM user WHERE accept !='true' AND  (position = 'doctor' or position ='receptionist' or position = 'technician' or position ='pharmacist' or position ='madmin') ORDER BY user_id ASC";
							$result1=mysql_query($query);
							$i =0;
							$count=0;
						}
						if(($_SESSION['res']=="radmin")||(in_array("radmin",$_SESSION['position1'])))
						{
							$query ="SELECT * FROM user WHERE accept!='true' AND (position= 'radmin' or position = 'student' or position = 'head' or position ='sadmin') ORDER BY user_id ASC";
							$result1=mysql_query($query);
							$i =0;
							$count=0;
						...
						if(($_SESSION['res']=="padmin")||(in_array("padmin",$_SESSION['position1'])))
						{
							$query ="SELECT * FROM user WHERE accept!='true' and position ='padmin' ORDER BY user_id ASC";
							$result1=mysql_query($query);
							$i =0;
							$count=0;
						}
					...
					<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
						...
						<?php 
							...
							{	
								...
								if(($_SESSION['res']=="mainadmin")||(in_array("mainadmin",$_SESSION['position1'])))
								{
									echo '<td width="1">'.$query_row['action'].'<input hidden size="1" type="text" value ="'.$query_row['action'].'"  name ="actions'.$i.'"/></td>';
								}
								...
								/*accept ekema */
								echo '<td width="1" hidden>'.$query_row['accept'].'<input size="1" type="text" hidden value ="'.$query_row['accept'].'"  name ="accepts'.$i.'"/></td>';//accept eke true false checking
								echo  '<input type="checkbox" checked hidden name="check-all'.$i.'" />';
								echo '<td>';		  
								echo '<button type="submit"  width="1" class=" center btn btn-primary glyphicon glyphicon-edit" title="Save" value ="Save" name ="save'.$i.'"></button>';
								echo '<button type="submit"   class=" btn btn-danger glyphicon glyphicon-trash"  title="Delete" value ="Delete" name="deletebtn'.$i.'"></button>';/*delete confirm*/
								if(isset($_POST['save'.$i.'']))
								{
									if(isset($_POST['check-all'.$i.'']))
									{
										...
										if(!$result)
										{
											echo "<script type='text/javascript'>alert('Upadte is faild');\"New_Profiles.php\"</script>";
										}
										else
										{
											echo "<script>alert('Upadte Account');window.location = \"New_Profiles.php\"</script>";
											echo $test;
										}
									}
									else
									{
										echo "<script type='text/javascript'>alert('Select the check box');window.location = \"New_Profiles.php\"</script>";
									}
								}
										...
										echo"<script type='text/javascript'>
												{
													...
													if(a)
													{
														window.location.href='delete_N_P.php?user_id='+".$user_id."
													}
												}
											</script>";	
									}
								...
								<button type="submit"  width="1" class=" center btn btn-success glyphicon glyphicon-pencil" title="Edit" id="edit" value ="'.$query_row['user_id'].'+'.$i.'" name ="edit"></button>
								</form>';
								echo '</td>';
									echo '</tr>';
							}
