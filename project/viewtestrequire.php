...
if(isset($_POST['search']))
{
	...
	$quary="select * from test_require where stregno LIKE '%".$valuetosearch."%'";
	$searchresult=filtertable($quary);
}
else
{
	$quary="select * from test_require";
	$searchresult=filtertable($quary);
}
...
function handleCheck(y)
        {
            ...
            xhttp.open("GET", "editcheck.php?x="+y, true);
            xhttp.send();
        }
</script>
...
<center><form action="viewtestrequire.php" method="post"></center>
...
<td><span id="error-notices"></span>
<input type="checkbox" id="checklist" <?php echo $a;?> onclick="handleCheck('<?php echo $row["stregno"]; ?>')" name="checklist" ></td>
</tr>
<?php endwhile; ?>
