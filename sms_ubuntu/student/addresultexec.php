...
if(mysql_num_rows($resultinno)> 0 && mysql_num_rows($resultcode)> 0) {
	if($id == 0){
	  $sql = "INSERT INTO results (inum, unit, credit, grade, points)"
		...
		mysql_query($sql);
		mysql_query("INSERT INTO log (RegNo, lname, position,action) VALUES ('$reg_no','$l_name','head','".$unit." for ".$indNo." results has been added')");
	}
	  ...
	  $sql = "UPDATE results "
		   ...
		   . " WHERE id       = " . $id;
		mysql_query($sql);
		mysql_query("INSERT INTO log (RegNo, lname,position,action) VALUES ('$reg_no','$l_name','head','".$unit." for ".$indNo." results has been updated')");
	}
